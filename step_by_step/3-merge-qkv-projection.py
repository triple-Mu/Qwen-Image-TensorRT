...
class FusedQKVAttnProcessor:
    ...
    def __call__(
            ...
            image_rotary_emb: Optional[Tuple[
        ...
        # [b, txt_seq_len, 3 * h]
        txt_qkv = attn.to_added_qkv(encoder_hidden_states)
        # [b, img_seq_len, 3, n, d]
        img_qkv = img_qkv.unflatten(-1, (3, attn.heads, -1))
        # [b, txt_seq_len, 3, n, d]
        txt_qkv = txt_qkv.unflatten(-1, (3, attn.heads, -1))
        # [b, img_seq_len, n, d]
        img_query, img_key, img_value = img_qkv.unbind(2)
        # [b, txt_seq_len, n, d]
        txt_query, txt_key, txt_value = txt_qkv.unbind(2)
        ...
        joint_hidden_states = scaled_dot_product_cudnn_attention(
            joint_query,
            joint_key,
            joint_value,
        )
...
class OptDit(nn.Module):
    def __init__(self, dit: QwenImageTransformer2DModel):
        ...
        for m in dit.modules():
            if isinstance(m, Attention) and not m.fused_projections:
                m.fuse_projections()
                m.set_processor(FusedQKVAttnProcessor())
        self.dit = dit
